...
cluster7 <- findClusters(clustered, rho=rho7, delta = delta7)
cluster8 <- findClusters(clustered, rho=rho8, delta = delta8)
```
...
taxonomy$cluster7 <- cluster7$clusters
taxonomy$cluster8 <- cluster8$clusters
```

```{r}
report_clusters <- function(pool) {
  c(nrow(unique(pool[,c('cluster1')])), nrow(unique(pool[,c('cluster2')])), nrow(unique(pool[,c('cluster3')])), nrow(unique(pool[,c('cluster4')])), nrow(unique(pool[,c('cluster5')])), nrow(unique(pool[,c('cluster6')])), nrow(unique(pool[,c('cluster7')])), nrow(unique(pool[,c('cluster8')])))
}

report_clusters2 <- function(pool) {
  c(length(unique(pool[,c('cluster1')])), length(unique(pool[,c('cluster2')])), length(unique(pool[,c('cluster3')])), length(unique(pool[,c('cluster4')])), length(unique(pool[,c('cluster5')])), length(unique(pool[,c('cluster6')])), length(unique(pool[,c('cluster7')])), length(unique(pool[,c('cluster8')])))
}
```
...
locallity_cluster_scores <- function(test_localities, regional_pool_clusters) {
  ...
  test_localities$cluster_score7 <- 0
  test_localities$cluster_score8 <- 0
  ...
  for (row in 1:nrow(test_localities)) {
    ...
    test_localities$cluster_score7[row] <- cluster_scores[7]
    test_localities$cluster_score8[row] <- cluster_scores[8]
  }
...
```
```{r}
plot_output('cluster_score7', rho7, delta7)
```

```{r}
plot_output('cluster_score8', rho8, delta8)
```


```{r}
write_csv(taxonomy[,c("scientific_name", "cluster1", "cluster2", "cluster3", "cluster4")], "result.csv")
```
